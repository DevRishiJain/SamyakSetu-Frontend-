            ...
            <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl p-8 lg:p-10 shadow-2xl relative z-10 border border-primary/20">
                <div className="flex flex-col items-center mb-8">
                    ...
                    <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-50 mb-2">
                        {isLogin ? t('auth.welcomeBack') : t('auth.joinSamyakSetu')}
                    </h2>
                    <p className="text-slate-500 text-center text-sm">
                        {isLogin ? t('auth.loginSubtext') : t('auth.signupSubtext')}
                    </p>
                </div>
                ...
                <div className="mb-5 p-3 bg-saffron/10 border border-saffron/30 rounded-xl text-saffron text-xs flex items-start gap-2">
                    ...
                    <div>
                        <span className="font-bold">{t('auth.prototypeMode')}</span> {t('auth.prototypeInfo', { otp: '000000' })}
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                    {!isLogin && (
                        <div>
                            <label className="block text-sm font-semibold mb-1 text-slate-700 dark:text-slate-300">{t('auth.fullName')} ({t('auth.fullNameHi')})</label>
                    ...
                    <div>
                        <label className="block text-sm font-semibold mb-1 text-slate-700 dark:text-slate-300">{t('auth.phoneNumber')} ({t('auth.phoneNumberHi')})</label>
                        <div className="relative flex">
                            ...
                            <input
                                ...
                                placeholder={t('auth.tenDigitNumber')}
                            />
                        </div>
                    </div>

                    <div>
                        <label className="block text-sm font-semibold mb-1 text-slate-700 dark:text-slate-300">{t('auth.otpVerification')} ({t('auth.otpVerificationHi')})</label>
                        <div className="relative">
